...
if(isset($_POST['HHId']) && !empty($_POST['HHId'])){
    ...
    $query = " select    phone_numb  as serv_line_no, o.name, 		o.area_details as full_address,
                (select tt.name from task_types tt where tt.id = t.task_type_id) taskTypeId, q.comments ,  DATE_FORMAT( t.visit_date, '%Y-%m-%dT%H:%i')  visitDate  
                from households o   
		                ...
		                <!--        You can switch " data-color="orange" "  with one of the next bright colors: "blue", "green", "orange", "red", "azure"          -->
								...
								<div class="wizard-navigation">
								  ...
								  <div  class="col-sm-9 col-sm-offset-0" id="message"></div>
									...
									{
									     if ($_POST['taskTypeId'] == 12 )
									    {
									        $query =  "SELECT  p.id as id , p.name    from contacts_statuses  p   where p.id in (213,215,217,225,235,236,228,219,212,211,226,227,229,214,208,238)";
									    }
									    else  if ($_POST['taskTypeId'] == 4 || $_POST['taskTypeId'] == 5 )
									    {
									     
									        $query =  "SELECT  p.id as id , p.name    from contacts_statuses  p   where p.id in (213,217,225,235,228,219,212,211,226,227,229,214,208,237,238) ";
									     }
									    
									    else
									    {
									        $query =  "SELECT  p.id as id , p.name    from contacts_statuses  p   where p.id in (213,217,225,235,228,219,212,211,226,227,229,214,208,238) ";
									    }
									...
									?>
									  ...
									  <div  class="col-sm-6 col-sm-offset-3" >
									   <label   for="otherComment" style="font-size: 20px;color:red;" >*Remarks</label>
									 	 <textarea maxlength="250" class="form-control" id="otherComment" rows="12" class="form-control" required ><?php /*echo $quotaData["comments"];*/?></textarea>
									  </div>
		                        ...
		                        <div class="wizard-footer">
		                          ...
		                          if ($_POST['taskTypeId'] == 12 || $_POST['taskTypeId'] == 11 || $_POST['taskTypeId'] == 10 || $_POST['taskTypeId'] == 4 || $_POST['taskTypeId'] == 15 || $_POST['taskTypeId'] == 17 || $_POST['taskTypeId'] == 20
		                              || $_POST['taskTypeId'] == 22 || $_POST['taskTypeId'] == 24 ) {?>
 										<div  class="col-sm-6 col-sm-offset-3" >
 										 <label   for="visitDate" style="font-size: 20px;" >Time of Visit</label>
									  ...
									  <div  class="col-sm-10 col-sm-offset-4" id="message"><small>*** You cannot add the Visit time in this step! </small></div>
									   <?php  }  ?>
       ...
       //alert (encodeURI('taskId='+taskId+'&statusId='+ statusRadio+'&otherComment='+  otherComment +'&ogeroId='+ogeroId+'&userId='+userId+'&action=updateContacts'));
        ...
        if (typeof statusRadio  == "undefined"  ||  statusRadio === "" ||  otherComment ===  "undefined" ||  otherComment === "" ) {
            ...
            $.ajax({
                type: "POST",
                url: "getServices.php",
                data: encodeURI('taskId='+taskId+'&statusId='+ statusRadio+'&otherComment='+  otherComment +'&ogeroId='+ogeroId+'&userId='+userId+'&visitDate='+visitDate+'&action=updateContacts'),
                dataType: 'JSON' ,
