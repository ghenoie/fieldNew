	        ...
	        <div class="row">
		        <div class="col-sm-12 col-sm-offset-0">
		            ...
		            <div class="wizard-container">
		                ...
		                <div class="card wizard-card" data-color="red" id="wizardProfile" style="min-height: 910px;">
								...
								<div class="wizard-navigation">
								  ...
								  <div  class="col-sm-9 col-sm-offset-0" id="message"></div>
									...
									<?php 
									    ...
									    {
									        $query =  "SELECT  p.id as id , p.name from statuses p  where id in (3,9,10,20,21,22,23)";
									    }
									    else if ($_POST['taskTypeId'] == 12)
									    {
									        $query =  "SELECT  p.id as id , p.name from statuses p  where id in (2,3,4,10,20)";
									    }
									    else if ($_POST['taskTypeId'] == 4 || $_POST['taskTypeId'] == 15 || $_POST['taskTypeId'] == 17 || $_POST['taskTypeId'] == 17|| $_POST['taskTypeId'] == 20 || $_POST['taskTypeId'] == 22 || $_POST['taskTypeId'] == 24)
									    {
									        $query =  "SELECT  p.id as id , concat ('Incentive Delivery: ', p.name) name from statuses p  where id in ( 3,4,10,19  );";
									    }
									...
									?>
									  ...
									  <div  class="col-sm-6 col-sm-offset-3" >
									   <label   for="otherComment" style="font-size: 20px;color:red;" >*Remarks</label>
									 	 <textarea maxlength="750" class="form-control" id="otherComment" rows="10" class="form-control" required ><?php echo $quotaData["comments"];?></textarea>
									  </div>
									  	...
									  	<div  class="col-sm-6 col-sm-offset-3" >
    									    ...
    									    <?php 
									          $query =  "SELECT  p.id as id , p.name from users  p  where id in (10)";
    									      $statusResult = $ds->select($query);
    								         foreach ($statusResult as $status) {
    									       $selected ='';
									       ...
									       echo "<div class='radio radio-info'><input type='radio' name='technicianRadio'  value='".$status["id"] ."' required  $selected checked='checked'><label> " .$status["name"] ." </label></div>";
									 	   }
									       ?>	
									  	...
									  	<div  class="col-sm-6 col-sm-offset-3" >
    									 	<label   for="visitDate" style="font-size: 20px;color:red;" >*Time of Visit</label>
    									 	 <input  class="form-control"  id="visitDate"  type="datetime-local" name="visitDate"   min="2020-12-30 08:30"  value="<?php if (!empty( $quotaData["visitDate"])) echo   $quotaData["visitDate"] ; ?>"> 
									  </div>
									  ...
									  <div  class="col-sm-6 col-sm-offset-3" >
									    ...
									    <div  id="giftDiv" class="col-sm-6 col-sm-offset-3" >
    									 	<label   for="giftId" style="font-size: 20px;color:red;" >*Gift</label>
    									 	
    									 	  <?php 
									          $query =  "SELECT id , name , img  FROM umshini_bo.gifts  where status_id = 2";
    									      $statusResult = $ds->select($query);
    								         foreach ($statusResult as $status) {
    									       $selected ="checked='checked'";
//     									       if (isset($quotaData["technicianId"]) && ($quotaData["technicianId"]) >  0)
...
//     									       }
									   ...
									   //    echo "<div class='radio radio-info'><input type='radio'  name='giftId'  value='".$status["id"] ."' required  $selected ><label> " .$status["name"] ." </label></div>";
									       
									       
									       
									       echo "<span>
    									 	<div class='hover-img'><font face='Arial'>
    									 	<input  type='radio' value='".$status["id"] ."' name='giftId' required  $selected  ><font> " .$status["name"] ." </font><span><img src='" .$status["img"] ."' alt='image' height='200'/></span>
    									 	</div></span><br />";
									 	   }
									       ?>	
									    ...
									    <div id="ticketDiv" class="col-sm-6 col-sm-offset-3" >
    									 	<label   for="ticketCount" style="font-size: 20px;color:red;" >*Number of Tickets</label>
    									 	 <input  class="form-control"  id="ticketCount"  type="number" name="ticketCount"  value="<?php if (!empty( $quotaData["ticketCount"])) echo   $quotaData["ticketCount"] ; ?>"> 
									  </div>
    ...
    $("#updateStatus").click(function () {
       ...
       if (r == true) {
		...
		var ogeroId = <?php echo  $_POST['HHId']?>;
		  taskId = <?php echo  $_POST['taskTypeId']?>; 
        ...
        if ( (taskId != 4
				&&(typeof statusRadio  == "undefined"  ||  statusRadio === "" ||  otherComment ===  "undefined" ||  otherComment === "" || technicianRadio == "undefined"  ||  technicianRadio === "" || visitDate == "undefined"  ||  visitDate === ""   
					|| orderNbId == "undefined" ||  orderNbId === "")
				)
			||
			...
			) 
        {
...

         {
            ...
            $.ajax({
                type: "POST",
                url: "getServices.php",
                data: encodeURI('taskId='+taskId+'&statusId='+ statusRadio+'&otherComment='+  otherComment +'&ogeroId='+ogeroId+'&userId='+userId+'&visitDate='+visitDate+'&action=updateHousehold'+'&techUserId='+ technicianRadio+"&orderNbId="+orderNbId+"&giftId="+giftId+"&ticketCount="+ticketCount),
                dataType: 'JSON' ,
