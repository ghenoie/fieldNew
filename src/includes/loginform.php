...
class LoginForm extends DbConn
{
    public function checkLogin($myusername, $mypassword)
    {
        ...
        if ($curr_attempts >= $max_attempts && $timeDiff < $login_timeout) {
            ...
            if (password_verify($mypassword, $result['password']) && $result['verified'] == '1') {
                ...
                $success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>Your account is inactive</div>";
    ...
    public function insertMembersLogs($userId, $access_type)
    {
        try {
            $db = new DbConn;
      
            $tbl_memberslog = $db->tbl_memberslog ;
            $datetimeNow = date("Y-m-d H:i:s");
           
            $stmt = $db->conn->prepare("INSERT INTO ".$tbl_memberslog." (  user_id, access_datetime, access_type) values(:userId, :datetimeNow, :access_type)");
